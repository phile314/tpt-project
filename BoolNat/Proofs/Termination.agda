...
termination {ty} H (t <- t₁) | Halts (vref x₁) H2 xs | Halts v H3 x with elem? H3 x₁ ty
termination {ty} H (t <- t₁) | Halts (vref x₁) H2 xs | Halts v H3 x | inj₁ x₂ = {!x₂!} -- Halts v (replace H3 x₂ v) (((E-AssignL* xs) ++ E-Assign* x) ++ [ (E-AssRed-Suc {ty} {{!!}} refl x₂) ] )
termination H (t <- t₁) | Halts (vref x₁) H2 xs | Halts v H3 x | inj₂ y = {!!} -- Here case analyses using use? 
