...
data Step : forall {ty S1 S2} -> {H1 : Heap S1} -> {H2 : Heap S2} -> {s : S1 ⊆ S2} -> Δ s H1 H2 -> Term ty -> Term ty -> Set where
 ...
 E-NewVal     : ∀ {ty S} {H : Heap S} {v : Term ty} {isV : isValue v} -> 
                Step (Allocate {v = v} {isV = isV} (Same H)) (new v) (ref (Top {Cons ty S}))
 ...
 E-DerefVal   : forall {S S' H ty} {e : Elem S' ty} {isP : S' ⊆ S} -> 
                Step {ty} (Same H) (! (ref {S'} e)) (lookup H (weaken isP e))
 ...
 E-AssRed     : forall {ty S S'} {H : Heap S} {v : Term ty} {isV : isValue v} {e : Elem S' ty} {isP : S' ⊆ S} ->
