...
verr-is-err {.Boolean} {true} ()
verr-is-err {.Boolean} {false} ()
verr-is-err {ty} {error} unit = unit
verr-is-err {.Natural} {num x} ()
verr-is-err {.Boolean} {iszero t} {()} isVE
verr-is-err {ty} {if t then t₁ else t₂} {()} isVE
verr-is-err {(Ref ty)} {new t} {()} isVE
verr-is-err {ty} { ! t} {()} isVE
verr-is-err {ty} {t <- t₁} {()} isVE
verr-is-err {Ref ty} {ref x} ()  
verr-is-err {ty} {t >> t₁} {()} isVE
verr-is-err {ty} {try t1 catch t2} {()} isVE


-- Big step to small step conversion.
...
-- Elem proofs are valid for all equal heaps.
elem-heapeq : ∀ {ty r m n} {H1 : Heap m} {H2 : Heap n} -> Elem H1 r ty -> H1 ≅ H2 -> Elem H2 r ty
elem-heapeq {ty} {r} {.0} {_} {Nil} () eq
elem-heapeq {ty} {r} {suc n₁} {.0} {Cons v H1} {Nil} el ()
elem-heapeq {ty} {.0} {suc .n} {suc n} {Cons .v₁ .H2} {Cons v₁ H2} (Top {.n} {.ty} {.v₁} {.H2}) refl = Top {n} {ty} {v₁} {H2}
elem-heapeq {ty} {(suc i)} {suc .n} {suc n} {Cons .v₁ .H2} {Cons {ty'} v₁ H2} (Pop el) refl with elem-heapeq {H1 = H2} {H2 = H2} el refl
...
-- A BStep from value to value implies heap equality. (Maybe this lemma could be generalized?)
ref-use-⇓eq : ∀ {ty n m k l} {H1 : Heap n} {H2 : Heap m} {H3 : Heap k} {H4 : Heap l} {t1 t2 : Term ty} {v : Value ty}
...
ref-use-⇓eq E-Ref isV b2 b3 = prepend-step b2 b3
ref-use-⇓eq E-Error isV b2 b3 = prepend-step b2 b3
ref-use-⇓eq (E-IfTrue b1 b2) () b3 b4
ref-use-⇓eq (E-IfFalse b1 b2) () b3 b4
ref-use-⇓eq (E-IfErr b1) () b2 b3
ref-use-⇓eq (E-New b1) () b2 b3
ref-use-⇓eq (E-Deref b1) () b2 b3
ref-use-⇓eq (E-DerefErr b1) () b2 b3
ref-use-⇓eq (E-Ass rep b1 b2) () b3 b4
ref-use-⇓eq (E-AssOob x b1 b2) () b3 b4
ref-use-⇓eq (E-AssErr b1) () b2 b3
ref-use-⇓eq (E-Seq x b1 b2) () b3 b4
ref-use-⇓eq (E-SeqErr b1) () b2 b3
ref-use-⇓eq (E-TryCat x b1) () b2 b3
ref-use-⇓eq (E-TryCatEx b1 b2) () b3 b4
...
... | bstp' = E-Ass rep bstp' (ref-use-⇓eq bstp isV s bstp₁)
prepend-step {H1 = H1} (E-AssRight (isV , notE) s) (E-AssOob x bstp bstp₁) with val-of-val-inv isV bstp H1
... | bstp' = E-AssOob x bstp' (ref-use-⇓eq bstp isV s bstp₁)
...
prepend-step (E-SeqVal {ty1 = ty1} {t1 = t1} (proj₁ , proj₂)) b = E-Seq (λ x → proj₂ (verr-is-err x)) (value-of-term proj₁) b
